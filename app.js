...
let fixture=FALLBACK_FIXTURE,artifacts={},active='sql';
...
generateBtn.onclick=()=>{
 try{
  generateBtn.disabled=true;generateBtn.textContent='Analyzing metadata context…';
  ...
  else if(q.includes('legacy_segment')||q.includes('deprecat'))field=entity.fields.find(f=>f.name==='legacy_segment');
  else field=entity.fields.find(f=>f.name==='email')||entity.fields[0];
  if(!entity||!field)throw new Error('Unable to resolve target asset from available DataHub context.');
  ...
  console.error(err);risk.textContent='ANALYSIS ERROR';risk.style.color='#ff9c9c';output.textContent=`ContextForge could not complete analysis: ${err.message}`;
 }finally{generateBtn.disabled=false;generateBtn.textContent='Analyze DataHub context & generate patch'}
};
...
function generate(e,f,o){
 ...
 if(f.name==='legacy_segment'){return {
